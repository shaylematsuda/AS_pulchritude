...
```{r}
pamf <- pamf %>%
  filter(!uniqueID == "57")      # corals held overnight in bag with no holes / it died pre-CBASS...

pamf %>%
  filter(date == "2026-01-23" & basket_no == "43", var == "y_ii_")

pamf %>%
  filter(var == "y_ii_") %>%
  count(date, basket_no) %>%
  arrange(n)
# so everything has 16 data points - yay!
```
...
df <- pamf %>%
  ...
  dplyr::select(date, uniqueID, max_temp, aoi, f, fm, fvfmraw, fvfm)

df %>% count(uniqueID) %>% arrange(n)
df %>% filter(is.na(fvfm))
df %>% filter(fvfm == 0)

# Substitute Fv/Fm value of 0.0001 for zeros
# df <- df %>%
#   mutate(fvfm = case_when(fvfm == 0 ~ 0.0001, TRUE ~ fvfm))
...
initmods <- df %>%
  nest(data = c(max_temp, aoi, f, fm, fvfmraw, fvfm)) %>%
...
fmods <- dff %>%
  select(uniqueID, max_temp, aoi, f, fm, fvfmraw, problem, fvfm) %>%
  nest(data = c(max_temp, aoi, f, fm, fvfmraw, fvfm, problem)) %>%
  # Fit the model to each coral
  mutate(ll3 = map(data, tryll3)) %>%
  # Get model parameters and fitted values/residuals
  mutate(pars = map(ll3, tidy),
...
fvals <- fmods %>%
  ...
  unnest(pred) %>%
  full_join(fed50) %>%
  full_join(select(dff, uniqueID, max_temp, aoi, f, fm, fvfmraw, problem, fvfm)) %>%
  rename(ed50 = estimate)
```
...
ggplot(fvals, aes(x = max_temp, y = fvfm)) +
  ...
  facet_wrap(~uniqueID) +
  coord_cartesian(xlim = c(26, 50)) #set these boundaries to filter out the outliers for now

hist(ed50$estimate, breaks = 50)

#mean
ed50_mean<-fed50_filt %>%
  summarise(
...
vals_one  <- vals  %>% filter(uniqueID == id_example)
fed50_one <- fed50_filt %>% filter(uniqueID == id_example)

ggplot(vals_one, aes(x = max_temp, y = fvfm)) +
  geom_point(size = 2) +
  geom_line(aes(y = .fitted), linewidth = 1) +
  geom_vline(aes(xintercept = estimate), data = fed50_one, lty = 2) +
  geom_text(aes(x = estimate, y = 0.05, label = round(estimate, 2)),
            data = fed50_one, size = 4, nudge_x = -0.8) +
  coord_cartesian(xlim = c(26, 40)) +
  labs(
    x = "Temperature (°C)",
    y = "Fv/Fm",
    title = paste("Example CBASS Curve – UniqueID", id_example)
  ) +
  theme_classic()
```
